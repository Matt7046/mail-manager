...
Stesso schema di **Password Manager**: DNS su **GoDaddy** → VPS → **nginx Activity Manager** (conf in `/root/nginx-apps/`) + container API sulla rete Docker.
...
4. Compose AM: mount `/root/mail-manager/web:/var/www/mail-manager-web` + recreate `nginx`
5. `DEPLOY-ALL.BAT`
